...
void rdk_dbg_priv_log_msg(rdk_LogLevel level, const char *module_name, const char* format, va_list args)
{
    ...
    /* Bridge log message to OTEL LogRecord only within active trace spans */
    if (rdk_otlp_has_active_trace() && rdk_otlp_logs_enabled(module_name))
    {
...
static const char* rdk_format_log(const log4c_layout_t* layout, log4c_logging_event_t* event, rdk_LogFormat format)
{
    ...
    if (event->evt_buffer.buf_size > 0 && event->evt_buffer.buf_data != NULL)
    {
        ...
        if (n > -1 && n >= event->evt_buffer.buf_size && event->evt_buffer.buf_maxsize == 0)
        {
            ...
            char *new_data = (char *)realloc(event->evt_buffer.buf_data, new_size);

            if (new_data != NULL) {
                event->evt_buffer.buf_data = new_data;
